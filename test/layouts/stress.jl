...
@testset "data types" begin
    ...
    layout = Layouts.StressLayout(; optimal_distance, initial_locs=Layouts.rand_points_2d(100))
    @test Layouts.render_locs(graph, layout) isa Vector{<:Layouts.Point{2}}

    # with initial locations and mask
    layout = Layouts.StressLayout(; optimal_distance, initial_locs=Layouts.rand_points_2d(100), mask=trues(100))
    @test Layouts.render_locs(graph, layout) isa Vector{<:Layouts.Point{2}}

    # layered
    zlocs = rand([0,200], nv(graph))
    layout = Layouts.StressLayoutLayered(; zlocs, optimal_distance)
    @test layout isa Layouts.StressLayoutLayered
    @test Layouts.render_locs(graph, layout) isa Vector{<:Layouts.Point{2}}
end